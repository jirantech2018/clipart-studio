...
function v2DocumentSchemaSpec(worksheetMode = false): string {
  const stage4Kinds = [
    ...
    `- 모든 선택지·항목·낱말 필드는 반드시 학생이 실제로 읽을 수 있는 낱말/문장이어야 한다.`,
    `  다음 자리표시자 표기는 절대 금지: "(그림)", "(사진)", "(이미지)", "(예: 사과 그림)", "TODO", "TBD", "..." 등.`,
    `  이미지가 필요한 자리는 반드시 visualRequirement.needed=true 인 블록으로 만들고, label/text 필드에는 학생이 실제로 볼 낱말을 채운다.`,
    ...
    `  이 경우 primitive 를 image-observation 으로 바꾸거나 stem 을 재구성해 그림 관찰이 필수가 되게 한다. label 을 "(그림)" 같은 자리표시자로 대체하지 말 것 — 이 문서는 그대로 학생에게 인쇄된다.`,
