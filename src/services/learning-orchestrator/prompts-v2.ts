...
export function v2UserPromptContentPlan(context: GenerationContext): string {
  return [
    ...
    `  · 'auto' → 학습 목표와 문항 구성에 따라 시각자료를 적극적으로 설계한다.`,
    `    이미지가 학생 사고를 돕지 못하거나 정답을 부당하게 노출하게 될 문항만 visualPlan=null 로 허용한다.`,
    `    모든 문항에 이미지를 강제하지는 않는다 — 이미지가 실제로 도움이 되는 문항에만 배치한다.`,
    `- 학습 목표의 핵심 자체가 시각 인식·관찰·비교인 문항에서는 이미지가 사실상 필수다.`,
