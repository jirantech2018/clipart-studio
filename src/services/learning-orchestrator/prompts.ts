...
function multipleChoicePrompt(input: OrchestratorInput, common: string): string {
  return [
    ...
    ``,
    `필수 규칙 (모두 지킬 것):`,
    `- sections 배열의 첫 항목은 heading level 1 (자료 제목). 제목·본문 문항이 반드시 주제 "${input.topic}"과 일치.`,
    ...
    `- 힌트는 다음 규칙을 정확히 따를 것:`,
    `  · 초성 (자음, 첫소리) 문제 → "글자의 첫소리를 살펴보세요."`,
    `  · 중성 (모음, 가운데소리) 문제 → "글자의 가운데소리를 살펴보세요."  (절대 "첫소리·시작 소리·끝소리" 사용 금지)`,
    `  · 종성 (받침, 끝소리) 문제 → "글자의 받침 (끝소리) 을 살펴보세요."`,
    ``,
    `★ 주제 일치:`,
    `- 주제가 "내 이름 …" 처럼 학생 개인 활동을 요구하면 객관식이 부적합. 이 경우 주제를 자연스럽게 "글자에서 … 찾기" 로 재해석해서 문항을 만들되, 제목에서도 그 재해석을 반영.`,
    ``,
    `★ 세트 단위 다양성 (서버가 코드로 검증):`,
    `- ${input.questionCount}문항의 정답 내용이 서로 달라야 함 (같은 정답이 절반 초과 X).`,
    `- 정답 번호 (1/2/3/4) 도 한 위치에 몰리지 않게 골고루 분배 (한 위치 50% 초과 X).`,
    `- 자모 유형이면 문항마다 서로 다른 자모를 다룸.`,
    `  잘못된 예: 5문항 모두 ㅏ 만 다룸. 올바른 예: 1번=ㅏ, 2번=ㅓ, 3번=ㅗ, 4번=ㅜ, 5번=ㅣ`,
    `- 인접 두 문항의 선택지 배열이 완전히 같으면 안 됨 (선택지 순서·구성 다르게).`,
    `- stem 표현을 최소 2가지 이상 섞기.`,
    `  예: "‘가’에 모음은?" 형식만 반복 X, "모음 ㅏ가 들어간 글자를 고르세요.", "‘도’를 자음과 모음으로 바르게 나눈 것은?" 등 다양한 형식 혼합.`,
    ``,
    `★ 힌트 정책:`,
    `- 허용 힌트 예시:`,
    `  · "글자의 가운데소리를 살펴보세요."`,
    `  · "천천히 소리 내어 읽어보세요."`,
    `  · "첫소리 다음에 들리는 소리를 찾아보세요."`,
    `- 금지 힌트 (사용 시 서버가 거부):`,
    `  · "앞의 문제와 같아요."`,
    `  · "'가'와 비슷해요.", "'가', '나'와 같아요."`,
    `  · "앞에서 나온 답과 같아요."`,
    `- 힌트는 반드시 개념 설명이나 관찰 포인트여야 하며, 이전 문항 답을 참조하는 방식 금지.`,
    ``,
    input.grade <= 2
