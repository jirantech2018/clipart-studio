...
function commonMcRules(input: OrchestratorInput): string {
  return [
    ...
    ``,
    `★ 힌트 정책:`,
    `- 힌트는 학생이 어디를 살펴야 할지 방향만 안내. 개념 설명은 짧게, 정답을 정의하는 문구는 금지.`,
    `- 이전 문항 답을 참조하는 방식 금지: "앞의 문제와 같아요.", "'X'와 비슷해요.", "앞에서 나온 답과 같아요."`,
    `- 정답 텍스트·동의어·활용형을 힌트에 담지 마라 (서버가 코드로 검증).`,
    `- 자연스러운 힌트가 떠오르지 않으면 hint 필드를 아예 생략해도 된다.`,
    ``,
    input.grade <= 2
...
function koreanGeneralMcRules(input: OrchestratorInput): string {
  return [
    ...
    `- 자음·모음·초성·중성·종성·받침 관련 문항 금지.`,
    ``,
    `★ 자기완결성 (self-contained, 서버가 코드로 검증):`,
    `- 이 문서에는 이미지/사진/그림/삽화가 첨부되지 않는다.`,
    `- 아래 표현을 절대 stem 에 넣지 마라 (사용 시 서버가 거부):`,
    `  "이 사물", "이 그림", "위 그림", "아래 그림", "다음 그림", "다음 사진",`,
    `  "위 사진", "아래 사진", "화면에 있는", "그림을 보고", "그림에서", "그림 속",`,
    `  "다음 물건", "위 물건" 등 외부 자료를 가리키는 표현.`,
    `- 각 문항은 stem 안의 텍스트만 읽고 정답을 결정할 수 있어야 한다.`,
    `  잘못된 예 (금지):`,
    `    · "책상 위에 있는 것은?" → 무엇이 놓였는지 정보 없음, 오답 후보들도 책상 위 가능`,
    `    · "이 사물의 이름은?" → 대상 지시 실패`,
    `  올바른 예:`,
    `    · "글씨를 쓸 때 사용하는 물건은?" 선택지: [의자, 연필, 신발, 사과]`,
    `    · "‘꼬끼오’ 소리를 내는 동물은?" 선택지: [소, 개, 닭, 고양이]`,
    ``,
    `★ 문장 완결성:`,
    `- stem 은 반드시 완결된 물음 문장. 축약형 (예: "사과는 어떤?", "연필은 무엇?") 금지.`,
    `  잘못된 예 (금지): "연필은 무엇?"`,
    `  올바른 예: "연필은 어떤 물건인가요?" / "연필은 무엇을 할 때 사용하는 물건인가요?"`,
    ``,
    `★ 힌트 정책 (답 직접 노출 금지, 서버가 코드로 검증):`,
    `- 힌트에 정답 선택지 텍스트나 정답을 정의하는 명사구를 그대로 넣지 마라.`,
    `  잘못된 예 (금지):`,
    `    · 정답이 "닭 우는 소리"인데 힌트 "닭이 아침에 울어요." — 정답 노출`,
    `    · 정답이 "개 소리"인데 힌트 "개가 짖을 때 내는 소리예요." — 정답 노출`,
    `  올바른 예: "아침에 자주 들을 수 있는 동물 소리예요." / "우리 주변에서 자주 만나는 동물의 소리예요."`,
    ``,
    `★ 주제 "${input.topic}" 유형 예시:`,
    `- 낱말의 뜻: "다음 낱말과 뜻이 가장 비슷한 것은?" / "빈칸에 알맞은 낱말은?"`,
    `- 비슷한 말: "‘기쁘다’ 와 뜻이 비슷한 낱말은?" — 선택지에 후보 낱말`,
    `- 반대되는 말: "‘크다’ 와 뜻이 반대인 낱말은?"`,
    `- 낱말과 설명 연결: "다음 설명에 알맞은 낱말은? — 종이에 글씨를 쓰는 데 쓰는 도구"`,
    `- 상황에 알맞은 낱말: "다음 문장의 빈칸에 알맞은 말은? — 나는 배가 고파서 ___을(를) 먹었다."`,
    `- 사물 이름 읽기 (이미지 없이): "‘연필’을 소리 나는 대로 바르게 읽은 것은?" 또는 "다음 중 학용품의 이름은?"`,
    `- 흉내 내는 말: "‘꼬끼오’ 는 어떤 소리를 흉내 낸 말인가요?"`,
    ``,
    `★ 유형 다양성 (낱말·뜻 계열 자료):`,
    `- 5문항이면 최소 3가지 서로 다른 유형 사용. 예: 뜻 · 비슷한 말 · 반대말 · 낱말-설명 연결 · 상황 낱말 중 3개 이상.`,
    `- 같은 유형 (예: "‘꼬끼오’는?" + "‘멍멍’은?") 을 5문항 중 2개 초과 반복 금지.`,
    ``,
    `★ 주제 대상 범주 (매우 중요):`,
    `- 주제 "${input.topic}" 의 대상 범주 (사물 · 동물 · 소리 · 낱말 · 문장 · 이야기 등) 를 먼저 결정하고, 5문항 모두 그 범주 안에서만 만든다.`,
    `- 예: 주제 "주변 사물 이름 읽기" → 대상 범주는 "사물". 모든 문항의 stem·정답이 실제 사물 (연필/책/가방/신발/의자/컵 등) 관련.`,
    `  잘못된 예 (금지): 사물 주제에 "‘꼬끼오’ 소리는?" (동물 소리 범주) 를 섞기.`,
    `- 예: 주제 "동물 소리 알기" → 대상 범주는 "동물 소리". 사물 문항 금지.`,
    `- 대상 범주가 헷갈리면 주제의 명사구 마지막 부분을 기준: "사물 이름 …" → 사물, "낱말의 뜻 …" → 낱말/의미, "이야기 …" → 이야기/서사.`,
    ``,
    `★ 힌트 정책 v2 (의미 노출 금지):`,
    `- 힌트는 학생이 어디를 살피고 어떻게 생각할지 방향만 안내한다. 정답 그 자체 또는 정답을 정의하는 문구를 담지 마라.`,
    `- 정답의 동의어·활용형·풀어쓴 정의도 힌트에 넣지 마라.`,
    `  잘못된 예 (금지):`,
    `    · 정답이 "쓰는 것" 인데 힌트 "글씨를 쓸 때 사용하는 도구예요."   ← "쓰" 반복 + 정의`,
    `    · 정답이 "들고 다니는 것" 인데 힌트 "가지고 다닐 때 사용해요."   ← 동의어`,
    `    · 정답이 "읽는 것" 인데 힌트 "읽을 수 있는 물건이에요."          ← "읽" 반복 + 정의`,
    `  올바른 예:`,
    `    · "책상에서 자주 볼 수 있는 학용품을 떠올려 보세요."`,
    `    · "학교에 갈 때 물건을 담아 가는 데 쓰는 물건을 생각해 보세요."`,
    `    · "글씨나 그림이 담긴 물건이에요. 어디에서 자주 보나요?"`,
    `- 힌트는 각 문항의 필수는 아니다. 자연스러운 힌트가 떠오르지 않으면 hint 필드를 아예 생략해도 된다.`,
  ].join('\n');
}
...
function mathMcRules(input: OrchestratorInput): string {
  return [
    ...
    ``,
    `★ 자기완결성 (self-contained, 서버가 코드로 검증):`,
    `- 이 문서에는 이미지가 없다. "이 그림", "위 그림", "다음 사진", "화면에 있는" 같은 표현 금지.`,
    `- 세는 대상은 stem 안 문장으로 완전히 서술. 잘못된 예: "다음 그림에서 세어보세요."`,
    `  올바른 예: "사과가 5개, 배가 3개 있어요. 모두 몇 개인가요?"`,
    ``,
    `★ 주제 "${input.topic}" 에 맞는 수학 문항 유형 예시:`,
    `- 수 세기: "사과가 8개 있어요. 몇 개인가요?" 선택지는 숫자.`,
    `- 수의 크기 비교: "12와 21 중 더 큰 수는?" / "다음 중 가장 큰 수는?"`,
    `- 덧셈: "3 + 4 = ?" / "빵 5개와 우유 2개를 사면 모두 몇 개인가요?"`,
    `- 뺄셈: "7 - 2 = ?"`,
    `- 곱셈 (2학년): "5 × 3 = ?" / "한 접시에 3개씩 담긴 접시가 4개면 모두 몇 개?"`,
    `- 도형: "다음 중 세모 모양은?" (선택지는 도형 이름)`,
    `- 길이 비교 등`,
    ``,
    `★ 주제 일치 (매우 중요):`,
    `- "받아올림 덧셈" 주제면 반드시 두 자리 수 + 두 자리 수 (한 자리 올림 발생) 문항. 뺄셈·곱셈·수 세기 등 금지.`,
    `- "수 세기" 주제면 수를 세는 문항. 계산 문제 금지.`,
    `- 각 문항의 stem·선택지 모두 이 주제 범위 안에서만 구성.`,
  ].join('\n');
}
...
function individualActivitySubjectGuidance(input: OrchestratorInput): string {
  if (input.subject === 'MATH') {
    return [
      ...
      `- 이미지 없이도 학생이 풀 수 있게 상황을 문장으로 완결하게 서술. "가게 그림을 봐요" 같은 지시 금지.`,
      `- 자모·모음·자음 관련 문항 절대 금지.`,
      ``,
      `★ 활동별 주제 일치 (모든 activity 블록이 각각 검증됨):`,
      `- 주제 "${input.topic}" 이 "받아올림 덧셈" 이면 두 활동 모두 두 자리 수 + 두 자리 수 (한 자리 올림) 문제만 사용.`,
      `  잘못된 예 (금지): 활동 1은 27+15, 활동 2는 "한 접시에 3개씩 4개면?" (곱셈).`,
      `  올바른 예: 활동 1은 수식 표, 활동 2는 이야기 상황도 반드시 받아올림 덧셈`,
      `    "사과 27개가 있었어요. 15개를 더 담으면 사과는 모두 몇 개인가요?" 식·답 빈칸.`,
      `- 주제가 "수 세기" 이면 두 활동 모두 수를 세는 활동. 계산 문제 금지.`,
      `- 주제가 "곱셈구구" 이면 곱셈만. 덧셈/뺄셈 금지.`,
      ``,
      `★ 덧셈·뺄셈 표 예시:`,
      `  table headers=["문제","받아올림","답"], rows=[["27+15","",""], ["36+28","",""], ["45+19","",""]]`,
      `  → 문제 열은 실제 수식, 계산·답 열은 빈 문자열`,
      ``,
      `★ 상황 문제 예시 (같은 주제 유지):`,
      `  받아올림 덧셈이면 "바구니에 사과 27개가 있어요. 15개를 더 담으면 몇 개?"`,
      `  worksheet-table headers=["식","받아올림","답"], rowCount=1~2`,
    ].join('\n');
  }
