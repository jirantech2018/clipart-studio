...
// 멤버 + 초대 관리 화면. admin+ 만 렌더링 (라우터 가드 없이도 API 로 강제).
// UI 흐름:
//   상단: 조직 이름 (홈 링크)
//   섹션 1: 멤버 목록 (역할 dropdown + 강퇴 버튼)
//   섹션 2: 초대 폼 + pending 초대 목록 (링크 복사 + 취소)
...
export function MembersPage({ slug }: { slug: string }) {
  ...
  const { data: membersData, isLoading: membersLoading } = useOrganizationMembers(slug);
  const { data: invitesData } = useOrganizationInvites(slug);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">멤버 관리</h1>
