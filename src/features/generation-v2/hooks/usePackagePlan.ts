...
// 패키지 모드 AI 추천 훅.
//
// - 사용자 입력 (목적 / 주제 / 대상 / 스타일 / 요청 / usageChannels +
//   userAdded/Removed 키워드) 을 debounce (800ms) 후 /api/package-plan 에 던진다.
// - 최소 조건 (목적 · 대상 · 스타일 3개 필수) 미달이면 API 호출 자체를 안 함.
// - 응답 도착 시 상위 (컴포넌트) 가 mergePackagePlan 으로 store 를 갱신한다.
//   이 훅은 데이터 반환만 담당.
//
...
export interface UsePackagePlanInput {
  ...
  usageChannels: readonly UsageChannel[];
...
export function usePackagePlan(input: UsePackagePlanInput) {
  ...
  // 최소 조건: 목적/대상/스타일. 주제·usageChannels 는 optional 이지만
  // 값이 있으면 queryKey 에 포함되어 자동으로 재추천된다.
  const hasRequired =
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchPlan({
        ...
        usageChannels: [...debouncedInput.usageChannels],
