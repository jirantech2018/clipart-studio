...
#### M2 — 사용성·교육 품질 검증 후 확장 (v0.5 재조정, 사용자 지시 2026-09-08)

계획서 v0.4 의 "M2 = 3~6학년 seed 확장" 을 그대로 진행하지 않고, **M1 구조를 전체 학년으로 복제하기 전에** 짧은 사용성·교육 품질 검증을 먼저 수행한다. 기능 축소가 아닌 검증 후 확대.

**M2-1: M1 경험 보정 (UI 재설계)**
- [ ] **단원과 주제 필드 분리**
  - 단원: seed 기반 select (grade+subject 필터)
  - 주제: text input (자유 입력)
  - 단원만 선택되고 주제 비었을 때 → "이 단원의 세부 주제 추천받기" 버튼 노출
  - AI 추천 3개 카드 클릭 시 주제 필드 자동 채움 + **사용자가 문구 수정 가능**
- [ ] **AI 추천 카드 명칭 변경** (내부 값 basic/realworld/inquiry 는 유지)
  - basic → **기본 개념 익히기**
  - realworld → **생활 속에서 적용하기**
  - inquiry → **생각을 넓혀 탐구하기**
- [ ] **입력 방식 재배치**
  - 직접 입력이 기본 흐름
  - AI 추천은 "주제를 정하지 못했을 때 도움받는 보조" 위치 (별도 분리된 방식 X)
- [ ] **기존 `/generate` 와 시각적 통일**
  - 페이지 폭·여백·카드·입력창·버튼·선택/로딩/오류 상태·크레딧 표시·미리보기·다운로드 영역
...
- [ ] 아래 고정 조건으로 표본 생성:
  ...
  - 1학년 수학 객관식 2
  - 2학년 수학 개별활동지 2
- [ ] 결과 표 (열): 단원 · 주제 · 추천 유형 · 문항 수 · 사용 어휘 수준 · 정답 오류 여부 · 동일 문항 반복 여부 · AI 자체 검증 결과 · PDF 링크
- **완료 조건**: 사용자가 표본 검토 후 프롬프트·학년별 표현 기준 확정

**M2-3: 검증 후 확장 (M2-1 + M2-2 승인 후)**
- [ ] 3~6학년 국·수 seed 추가
- [ ] 나머지 자료유형 3종 프롬프트 완성 (OX 퀴즈 · 개념 정리 · 읽기 자료)
- [ ] 자료유형별 안내 문구 (§2.4 매트릭스)
- [ ] 학년별 어휘·문항 난이도 정책 적용

**사이드바 nav "학습지 만들기" 노출 조건 (M2 종료 게이트)**
아래 4개 모두 통과 시 노출:
- 표본 품질 확인 (M2-2)
- `/generate` 스타일 통일 (M2-1)
- 생성 실패 및 크레딧 환불 동작 확인
- 모바일 기본 사용성 확인
